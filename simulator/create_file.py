...
def complete_template( current_row, time ): #prepare the file .json for a single app, the current row input indicates at which row we have to start
    ...
    for i in range(current_row, current_row + n_ws):
        ...
        if gamma > 0 :
            data["workload"][f"class{i}"]["mu_min"] = float(lines_ws[i][3]) - float(lines_param[1][5]) * float(lines_ws[i][3])
            data["workload"][f"class{i}"]["mu_max"] = float(lines_ws[i][3]) + float(lines_param[1][5]) * float(lines_ws[i][3])
            data["workload"][f"class{i}"]["delay_min"] = float(lines_ws[i][2]) - float(lines_param[1][6]) * float(lines_ws[i][2])
            data["workload"][f"class{i}"]["delay_max"] = float(lines_ws[i][2]) + float(lines_param[1][6]) * float(lines_ws[i][2])
            gamma -= 1
        else :
            data["workload"][f"class{i}"]["mu_min"] = float(lines_ws[i][3])
            data["workload"][f"class{i}"]["mu_max"] = float(lines_ws[i][3])
            data["workload"][f"class{i}"]["delay_min"] = float(lines_ws[i][2])
            data["workload"][f"class{i}"]["delay_max"] = float(lines_ws[i][2])
    with open(path_template+"/FogDaniloConfig.json", "w") as file:
        json.dump(data, file)
...
def run_app(current_row):
    complete_template(current_row,time)
    single_simulation(time)
    os.system("cp *.data /home/simula/dief_2018_fog_simulator/omnetpp-5.4.1/samples/Fog/simulations/A_Robust_Game_for_On_Spot_Price_Cloud_Markets\
                /instances/saas_robust_game"+str(time))
