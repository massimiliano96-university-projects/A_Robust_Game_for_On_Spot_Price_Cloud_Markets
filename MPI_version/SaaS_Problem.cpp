...
void SaaS_Problem::solve(void)
{
  try
  {
    ...
    std::vector<GRBLinExpr> bx; //

    std::vector<GRBVar> x_v;              // here we store the variables X_a_w because we need them for the computation of the parameters B
    std::vector<GRBVar> y_v;        // here we store the variables epsilon because we need it for the robust constraints
    std::vector<GRBVar> alpha_v;
    std::vector<GRBVar> beta_v;
    ...
    for(unsigned i = 0; i < s -> get_size(); i++)                                      // cicle over all the applications of the SaaS
    {
      ...
      total_on_flat += r; // creating the sum of r_a's
      total_VM += ( r + d );
      ...
      */
      for(unsigned j = 0; j < app.get_size(); j++)                                                  // cicle over all the WSs of the application i
      {
        ...
        //GRBVar epsilon = model.addVar(0.0, GRB_INFINITY, 0.0  , GRB_CONTINUOUS);                    // creating epsilon_a_w
        //epsilon_v.push_back( epsilon );                                                             // we store epsilon_a_w in a vector in order to use it with robust constraints

        //model.addConstr( x <= epsilon );                                                            // X_a_w <= epsilon_a_w
      ...
      // here i add the robust constraints
      for(unsigned j = 0; j < app.get_size(); j++)                                       // cicle over all the WSs of the application i
      {
        auto web_service = app.get_web_services()[j];                                    // j-th WS of the i-th application
        ...
        auto  B_a_w = set_param_B(web_service, app);                                     // I compute the parameters B_bar and B_hat using the function set_param_B
        ...
        for(unsigned l = 0; l < B_a_w.size(); l++)                                                // cicle over WS l of the application i
        {
          ...
          betas += beta;                                                                          // computng the sum of all beta's
...
*/
    ...
    for(unsigned i = 0; i < (*s).get_size(); i++)                                            // cicle over all the applications of the SaaS
    {
      ...
      //s->set_desired_on_spot( app,  std::min( s -> get_on_demand(app) , eta_j/(1-eta_j) * ( s -> get_on_demand(app) + s -> get_on_flat(app) ) ));
...
void SaaS_Problem::update (void) //this function transform the given_on_spot VM in on_spot VM
{
...
void SaaS_Problem::rounding( void ) // this function perform a rounding of the results in a smart way
{
  auto apps = s -> get_applications();
