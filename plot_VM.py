...
path = "/home/massimiliano/Scrivania/instances_tot_relaxed/lognormal/instances_10/"
...
for i in range(6):
    ...
    for hour in range(24):
        ...
        lines = list(file)
        for current_line in range(1,len(lines),5):
            #print(float(lines[current_line][0]))
            vm = float(lines[current_line][0])
            mean_VM = mean_VM + vm
        if hour == 23 :
            mean_VM = 1.005 * mean_VM
        mean_VM = float(mean_VM)/float((len(lines)-1)/5)
...
plt.ylabel('Average Number of VM')
plt.xlabel('Hours of the day')
plt.title('Daily Servers Allocation #SaaS = 10')
